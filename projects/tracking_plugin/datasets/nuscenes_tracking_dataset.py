...
class NuScenesTrackingDataset(NuScenesDataset):
    ...
    def evaluate_tracking(self,
        ...
        Returns:
            dict[str, float]: Results of each evaluation metric.
        """
        result_files, tmp_dir = self.format_tracking_results(results, jsonfile_prefix)
    ...
    def format_tracking_results(self, results, jsonfile_prefix=None):
        ...
        # refer to https://github.com/open-mmlab/mmdetection3d/issues/449
        if not ('pts_bbox' in results[0] or 'img_bbox' in results[0]):
            result_files = self._format_tracking_bbox(results, jsonfile_prefix)
        else:
            # should take the inner dict out of 'pts_bbox' or 'img_bbox' dict
            result_files = dict()
            for name in results[0]:
                ...
                tmp_file_ = osp.join(jsonfile_prefix, name)
                result_files.update(
                    {name: self._format_tracking_bbox(results_, tmp_file_)})
        return result_files, tmp_dir
    ...
    def _format_bbox(self, results, jsonfile_prefix=None):
        ...
        res_path = osp.join(jsonfile_prefix, 'results_nusc_detection.json')
    ...
    def _format_tracking_bbox(self, results, jsonfile_prefix=None):
        ...
        for sample_id, det in enumerate(mmcv.track_iter_progress(results)):
            ...
            boxes = lidar_nusc_box_to_global(self.data_infos[sample_id], boxes,
                                             ...
                                             tracking=True)
            for i, box in enumerate(boxes):
                name = det_class_names[box.label]
                if np.sqrt(box.velocity[0]**2 + box.velocity[1]**2) > 0.2:
                    if name in [
                            'car',
                            'construction_vehicle',
...
def lidar_nusc_box_to_global(info,
    ...
    for box in boxes:
        ...
        if tracking and classes[box.label] not in NuScenesTrackingDataset.TRACKING_CLASSES:
            continue
